...
public:
	...
	{
		SceneManager::Instance()->GetCamera()->SetPosition(Vector3(-2.0f, 7.5f, -30.0f));
		SceneManager::Instance()->GetCamera()->SetYaw(-160.f);
		SceneManager::Instance()->GetCamera()->SetPitch(10.f);
			...
			for (int y = 0; y < pyramid_stack_height; ++y)	
			{
				for (int x = 0; x <= y; ++x)
				{
					...
					Object* cube = CommonUtils::BuildCuboidObject(
						...
						true,
						colour);
					cube->Physics()->SetFriction(0.9f);
					cube->Physics()->SetElasticity(0.0f);	

					/*cube->Physics()->SetOnCollisionCallback([&](PhysicsObject* self_obj, PhysicsObject* other_obj)
					{
						return true;
					});*/
